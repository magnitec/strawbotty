...
export class TradingSystem extends MidaTradingSystem {
  ...
  async onStart() {
    console.log("TradingSystem: start");
  }

  async onTick(tick: MidaTick) {
    console.log(
      "TradingSystem.ts > getAccount",
      accountManager.getAccount().isDemo,
    );
  ...
  async onStop() {
    console.log("TradingSystem: stop");
  }
}
